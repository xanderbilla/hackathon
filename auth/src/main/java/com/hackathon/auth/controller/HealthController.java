...
public class HealthController {
    ...
    public ApiResponse<Map<String, Object>> getHealth() {
        ...
        try {
            ...
            healthData.put("database_name", "auth_db");
            healthData.put("status", "UP");
            return ApiResponse.success("Auth Service is healthy with database connectivity", healthData);
        } catch (Exception e) {
            healthData.put("database", "DOWN");
            healthData.put("database_error", e.getMessage());
            healthData.put("status", "UP");
            return ApiResponse.success("Auth Service is running but database connection failed", healthData);
        }
    }
    ...
    private void checkMySQLConnectivity() throws Exception {
        try (Connection connection = dataSource.getConnection()) {
            ...
            try {
                jdbcTemplate.queryForObject("SELECT SCHEMA_NAME FROM INFORMATION_SCHEMA.SCHEMATA WHERE SCHEMA_NAME = 'auth_db'", String.class);
            } catch (Exception e) {
    ...
    private void createAuthDatabase() throws Exception {
        try {
            ClassPathResource resource = new ClassPathResource("init-auth-db.sql");
            ...
            throw new Exception("Failed to create auth database: " + e.getMessage());
        }
    }
